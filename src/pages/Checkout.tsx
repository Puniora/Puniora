  ...
  const completeOrderCreation = async (pStatus: 'pending' | 'paid', pId: string) => {
      try {
        ...
        const orderData = {
          ...
          payment_method: paymentMethod, // We normally need to add this column to schema if logic depends on it, but for now logic is inferred
          transaction_id: pId, // We might need to store this in a generic field or notes
          user_id: user?.id,
          items: items.map(i => ({
            id: i.product.id,
            name: i.product.name,
